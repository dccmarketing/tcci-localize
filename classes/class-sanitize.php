...
class TCCI_Localize_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'tcci-localize' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'tcci-localize' ) );
		 ...
		 */
		do_action( 'TCCI_LOCALIZE_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'TCCI_LOCALIZE_post_sanitize', $sanitized );
